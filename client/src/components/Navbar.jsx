...
const Navbar = () => {
  ...
  const { favoriteMovies, selectedCity, setShowCitySelector } = useAppContext()

  return (
    <div className="fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5">

      <Link to="/" className="max-md:flex-1">
        <img src={assets.logo} alt="CineSnap" className="h-[75px] w-auto" />
      </Link>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="max-md:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-primary/40 hover:border-primary text-sm text-gray-300 hover:text-white transition"
        >
          <MapPinIcon className="w-3.5 h-3.5 text-primary" />
          <span className="capitalize">{selectedCity || "Select City"}</span>
        </button>

        <SearchIcon className="max-md:hidden w-6 h-6 cursor-pointer" />
        {
          !user ? (<button onClick={openSignIn} className="px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull transition rounded-full font-medium">
            Login
          </button>) : (
            <UserButton>
